...
public static class TaskFunctionalExtensions
{
    public static async Task<Result<TOut>> MapTaskAsync<T, TOut>(this Task<Result<T>> task, Func<T, Task<Result<TOut>>> mapping)
    {
        var result = await task.HandleTaskCompletionAsync();
         ...
         return await result.MapResultAsync<TOut>(mapping);
    }
    ...
    public static async Task<Result> MapTaskAsync<T>(this Task<Result<T>> task, bool test, Func<T, Task<Result>> isTrue, Func<T, Task<Result>> isFalse)
    {
        var result = await task.HandleTaskCompletionAsync();

        return await result.MapResultAsync(test, isTrue, isFalse);
    }
